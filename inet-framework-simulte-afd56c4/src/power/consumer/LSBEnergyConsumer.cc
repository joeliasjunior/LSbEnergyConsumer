...
Define_Module(LSBEnergyConsumer);

LSBEnergyConsumer::LSBEnergyConsumer() :
    //isSleeping(false),
    energyConsumerId(-1),
...
void LSBEnergyConsumer::initialize(int stage)
{
...
void LSBEnergyConsumer::handleMessage(cMessage *message)
{
    updatePowerConsumption();
//    if (message == timer) {
...
void LSBEnergyConsumer::updatePowerConsumption()
{
    isSleeping = false;
...
//void LSBEnergyConsumer::scheduleIntervalTimer()
//{
